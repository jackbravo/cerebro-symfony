...
CREATE TABLE campaign (id INT AUTO_INCREMENT, nombre VARCHAR(255) NOT NULL, vendedor_id INT, categoria_id INT, producto_id INT, specialty_id INT, cliente VARCHAR(255), orden VARCHAR(255), facturacion DOUBLE, fecha_inicio DATETIME, fecha_cierre DATETIME, activa TINYINT(1) DEFAULT '1', fecha_ingreso DATETIME, fecha_actualizacion DATETIME, INDEX producto_id_idx (producto_id), INDEX categoria_id_idx (categoria_id), INDEX specialty_id_idx (specialty_id), INDEX vendedor_id_idx (vendedor_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE item (id INT AUTO_INCREMENT, campaign_id INT, plaza_id INT, categoria_id INT, tipo_id INT, responsable_id INT, prueba_color DATETIME, impresor_id INT, entrada_impresor DATETIME, salida_impresor DATETIME, mensajeria_id INT, guia VARCHAR(255), fecha_envio DATETIME, fecha_recepcion DATETIME, permisionario VARCHAR(255), carroceria VARCHAR(255), compra_directa VARCHAR(255), ruta VARCHAR(255), placas VARCHAR(255), economico VARCHAR(255), instalacion DATETIME, desmontaje DATETIME, evidencia TINYINT(1) DEFAULT '0', fecha_actualizacion DATETIME, INDEX campaign_id_idx (campaign_id), INDEX plaza_id_idx (plaza_id), INDEX categoria_id_idx (categoria_id), INDEX tipo_id_idx (tipo_id), INDEX impresor_id_idx (impresor_id), INDEX mensajeria_id_idx (mensajeria_id), INDEX responsable_id_idx (responsable_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
ALTER TABLE campaign ADD FOREIGN KEY (vendedor_id) REFERENCES sf_guard_user(id);
...
ALTER TABLE item ADD FOREIGN KEY (responsable_id) REFERENCES sf_guard_user(id);
...
ALTER TABLE item ADD FOREIGN KEY (campaign_id) REFERENCES campaign(id) ON DELETE CASCADE;
ALTER TABLE perfil ADD FOREIGN KEY (user_id) REFERENCES sf_guard_user(id);
